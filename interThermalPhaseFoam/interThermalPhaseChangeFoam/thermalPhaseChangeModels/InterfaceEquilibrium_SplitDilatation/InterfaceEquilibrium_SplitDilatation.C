...
void Foam::thermalPhaseChangeModels::InterfaceEquilibrium_SplitDilatation::calcPCV()
{
    ...
    // volScalarField nVolumeDimless = mesh_.V();
    // nVolumeDimless.dimensions().reset(dimless);

    dimensionedScalar dummyDim
    (
        "dummy",
    ...
    LiquidVolGen.ref() = LiquidVolGen.ref() * mesh_.V() * dummyDim;
    VaporVolGen.ref() = VaporVolGen.ref() * mesh_.V() * dummyDim;
    ...
    PCVField.ref() = PCVField.ref() / mesh_.V() / dummyDim;
